...
export default function ErpSlider({
  ...
  const renderDataSrc = readyDatasrc.length > 1 ? readyDatasrc : pDataSrc;
      ...
      default:
        return (
          <BlockDiv customClassName="relative w-[406px] h-[506px] bg-white">
            <Image src={item?.mainimage || item?.iconImage} alt="" fill />
  ...
  return (
    <div className="flex flex-row justify-center items-center relative z-30 emergeny">
      ...
      <Swiper
        ...
        className="mySwiper"
      >
        {renderDataSrc?.map((item: any, index: number) => {
          return (
