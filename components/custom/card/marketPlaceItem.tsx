      ...
      >
        <div className="w-full h-[150px] relative">
          ...
          {data?.mainimage && (
            <span className=" ">
              <RenderAtom
                item={data?.position2 || { value: data?.mainimage }}
                renderType="image"
                customClassName={"w-full h-[150px] object-cover rounded-[10px]"}
              />
            </span>
          ...
          {data?.alttext && (
            <span className="absolute top-2.5 right-2.5 px-2 py-1 rounded-[20px] bg-[#ED975C] text-white text-xs font-semibold">
              {data?.alttext}
            </span>
          )}
        </div>
        <div className="flex flex-col p-2.5 space-y-2.5">
          ...
          />
          <RenderAtom
            item={data?.position3 || { value: data?.title }}
            renderType="title"
            customClassName={
          ...
          />
          <RenderAtom
            item={data?.position1 || { value: data?.description }}
            renderType="title"
            customClassName={
