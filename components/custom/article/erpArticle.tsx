...
import SurveySubscribe from "../../common/footer/surveySubscrive";
import LawArticle from "@/components/project/lawHelp/lawArticle";
  ...
  const renderContent = () => {
    switch (options?.type) {
      ...
      case "survey":
        return (
          <motion.div
            className={`flex flex-col mx-auto ${widgetnemgooReady?.widgetClassName}`}
          >
            <SurveySubscribe />
          </motion.div>
        );
      ...
      case "lawHelp":
        return (
          <motion.div
            className={`flex flex-col text-center ${widgetnemgooReady?.widgetClassName}`}
          >
            <LawArticle data={widgetnemgooReady} />
          </motion.div>
        );
      default:
        return (
          <motion.div
            ...
            className={`emerging-title flex flex-col mx-auto ${widgetnemgooReady?.widgetClassName} gap-y-2 animate-fade-out-up md:w-full lg:w-[1040px] md:px-0 xs:px-3`}
          >
            {/* {t(staticData?.title)} */}
            <RenderAtom
              item={{ value: t(staticData?.title) }}
            ...
            {staticData?.description && (
              <RenderAtom
                ...
                }  text-[16px] leading-[26px] inline-block xs:w-full`}
              />
            )}
            <style>
              {`
