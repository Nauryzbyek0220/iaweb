...
import MetaverseMasonry from "@/components/custom/metaverseWidget/metaverseMasonry";
import MainCard from "@/components/project/lawHelp/MainCard";
...
export default function ErpCard({
      ...
      <div className="flex flex-col justify-center w-full md:w-1/2 md:px-2 lg:py-0 xs:py-11 xs:px-1">
        <div className="pl-4 flex flex-col justify-evenly gap-5">
          ...
          <RenderAtom
            ...
            renderType="text"
            customClassName={`list-styled text-base text-[#7B7B93] text-left p-0 ${
              i === 2 ? ".." : ".."
            }  overflow-hidden`}
            customStyle={{
  ...
  // console.log("pDataSrc", pDataSrc);
  // console.log("ddss", widgetnemgooReady);
  // console.log("options options src ::", options);
    ...
    switch (options?.cardType) {
      ...
      case "masonry":
        if (options?.dataType === "metaverse") {
          return <MetaverseMasonry data={metaverseData} options={options} />;
        } else {
          return <Masonry data={readyDatasrc} options={options} />;
        }

      case "lawHelp":
          return <MainCard data={readyDatasrc} options={options} />;
      case "right":
        return (
        ...
        return (
          <div className="flex flex-col md:flex-row justify-center items-center md:justify-between gap-5 w-full">
              ...
              <RenderAtom
                ...
                item={{
                  value:
                    ...
                    staticItem?.imgurl ||
                    staticItem?.backgroundimage ||
                    staticItem?.imgmiddlethumb,
                }}
            ...
            <div className="flex flex-col justify-center items-center md:items-start gap-5 md:w-1/2">
              <RenderAtom
                renderType="title"
                item={{ value: t(staticItem?.title) || t(staticItem?.title) }}
                customClassName={`${options.titleClassName} xs:text-[1.5rem] md:text-[40px] text-center md:text-start font-bold md:leading-[46px]`}
              />
              <RenderAtom
                renderType="text"
                item={{
                ...
                customClassName={`${options.descClassName} md:text-[16px] sm:text-lg xs:text-base leading-[26px] font-normal w-full md:text-left xs:text-justify`}
              />
          ...
          <div className="flex flex-col md:flex-row justify-center items-center md:justify-between gap-10 w-full hContent">
            <div className="flex flex-col justify-center items-center md:items-start gap-5 md:w-1/2">
              ...
              <RenderAtom
                ...
                customClassName={`${options.descClassName} text-base leading-[30px] text-[#67748E] font-normal xs:text-justify list-styled `}
              />
              {staticItem?.extraDesription && (
                <div className="flex flex-col w-full space-y-8">
                  {staticItem?.extraDesription.map(
      ...
      case "imageCenter":
        return (
          <div className="md:flex-row justify-center items-center md:justify-between">
            <div className="flex items-center justify-center md:w-full p-5">
              <RenderAtom
                renderType="image"
                item={{
      ...
      case "gridColsCardCenter":
        return (
          <div className="flex flex-col w-full container mx-auto py-4">
            <div className={`flex flex-wrap justify-center`}>
              {renderData?.map((row: any, index: number) => {
                return (
                  <div
                    key={index}
                    className=" w-[32%] text-center xl:p-[30px] lg:p-[30px] xs:p-4 border-t-8 border-transparent rounded-t-[5px] rounded-b-[10px] space-y-3 cursor-pointer xl:min-w-auto lg:min-w-auto xs:min-w-[260px]"
                  >
                    <RenderAtom
                      item={{ value: row.iconimage }}
                      renderType="image"
                    ...
                    <RenderAtom
                      ...
                      item={{ value: t(row.description) }}
                      customClassName={
                        "md:text-[16px] sm:text-lg xs:text-base leading-[22px] text-[#fff] xs:text-justify"
                      }
                    />
                  </div>
                );
            ...
            >
              {renderData?.map((row: any, index: number) => {
                return (
                  <div
                    key={index}
                    className="flex flex-col xl:p-[30px] lg:p-[30px] xs:p-4 border-t-8 border-transparent rounded-t-[5px] rounded-b-[10px] space-y-5 cursor-pointer xl:min-w-auto lg:min-w-auto xs:min-w-[260px]"
                  >
                    <RenderAtom
                      item={{ value: row.iconimage }}
                      renderType="image"
      ...
      case "gridColsCard":
        ...
        return (
          <div className="flex flex-col w-full container mx-auto">
            ...
            >
              {renderData?.map((row: any, index: number) => {
                console.log("get data ::", row);
                const rows = row.integration?.split(",");
                const rowsId = row.integrationid?.split(",");
                console.log(rowsId);
                return (
                  <div
                    key={index}
                    className="flex flex-col xl:p-[30px] lg:p-[30px] xs:p-4 border-t-8 border-transparent rounded-t-[5px] rounded-b-[10px] space-y-5 xl:min-w-auto lg:min-w-auto xs:min-w-[260px]"
                  >
                    <RenderAtom
                    ...
                    <RenderAtom
                      ...
                      customClassName={
                        "text-[19px] font-bold leading-[30px] text-[#3C3C3C]"
                      }
                    />
                    <RenderAtom
                      renderType="text"
                      ...
                      customClassName={
                        "md:text-[16px] sm:text-lg xs:text-base leading-[30px] text-[#67748E] list-styled line-clamp-3"
                      }
                    />
                    <div className="flex flex-col">
                      <ul className="list-disc text-[#0C529D] underline ">
            ...
            {renderData?.map((item: any, index: number) => {
              return (
                <div
                  key={index}
                  className={`flex ${
                    startPosition || options?.startPosition == "right"
                      ? index % 2 != 0
                      ...
                      ? "flex-col md:flex-row-reverse"
                      : "flex-col md:flex-row"
                  } w-full container mx-auto justify-center items-center gap-10`}
                >
                  <div
                    className={`flex h-auto md:w-1/2 w-full ${
                  ...
                  <div className="flex flex-col  justify-center items-center md:items-start  md:w-1/2 xs:w-full gap-5">
                    ...
                    <RenderAtom
                      ...
                      customClassName="text-[#67748E] text-base list-styled leading-6 md:leading-8 font-normal"
                    />
                  </div>
                </div>
              );
